...
describe("User Model", () => {
  ...
  describe("validate", () => {
    ...
    it("should validate the roleId", ()=>{
      let user = new User({email: "xxx@xx.com", roleId:0, active:true, password:"xxx"});
      let [isValid, errs] = user.validate();
      expect(isValid).toBe(false);
      expect(errs).toHaveProperty("roleId");

      user = new User({email: "xxx@xx.com", roleId:4, active:true, password:"xxx"});
      [isValid, errs] = user.validate();
      expect(isValid).toBe(false);
      expect(errs).toHaveProperty("roleId");
